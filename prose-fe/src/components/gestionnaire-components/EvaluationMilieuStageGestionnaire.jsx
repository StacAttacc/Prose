...
import { evaluateWorkplaceForCandidature, getCandidaturesEnStage } from '../../services/GestionnaireService';
...
export default function EvaluationMilieuStageGestionnaire() {
    const { t } = useI18n();
    ...
    const loadCandidature = async () => {
        ...
        try {
            ...
            if (currentCandidature.evaluationMillieu) {
                ...
                // Pré-remplir avec les informations disponibles
                ...
                if (currentCandidature.stage) {
                    setFormData(prev => ({
                        ...prev,
                        nomEntreprise: currentCandidature.stage.employeur?.company || '',
                        addresse: currentCandidature.stage.location || '',
                        dateStage: currentCandidature.stage.startDate && currentCandidature.stage.endDate 
                            ? `${currentCandidature.stage.startDate} au ${currentCandidature.stage.endDate}`
                            : ''
                    }));
                }
            }
            ...
            setError(t('gestionnaire.erreurChargement') || 'Erreur lors du chargement de la candidature');
        } finally {
            setLoading(false);
        }
    };
    ...
    const validateForm = () => {
        ...
        if (missingField) {
            setError(t('gestionnaire.champRequis') || `Le champ ${missingField} est requis`);
            return false;
        }
        ...
        if (missingEvaluation) {
            setError(t('gestionnaire.evaluationRequis') || 'Toutes les évaluations sont requises');
            return false;
        }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            await evaluateWorkplaceForCandidature(candidatureId, evaluationData, user.token);

            setSuccess(true);
            setTimeout(() => {
                navigate('/gestionnaire/evaluations-milieu');
            }, 2000);
        } catch (err) {
            console.error('Erreur lors de la sauvegarde:', err);
            setError(err.response?.data?.message || err.message || t('gestionnaire.erreurSauvegarde') || 'Erreur lors de la sauvegarde');
        } finally {
            setSaving(false);
        }
    };
    ...
    if (!candidature) {
        return (
            <div className="container mx-auto px-4 py-8">
                <ErrorBanner message={error || t('gestionnaire.candidatureNonTrouvee') || 'Candidature non trouvée'} />
            </div>
        );
    }
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            <div className="mb-6">
                ...
                <h1 className="text-3xl font-bold text-gray-900 mb-2">
                    {t('gestionnaire.evaluationMilieuStage') || 'Évaluation du milieu de stage'}
                </h1>
                <p className="text-gray-600">
                    {t('gestionnaire.evaluantStage') || 'Évaluant le stage de'}: <span className="font-semibold">
                        {candidature.etudiant?.firstName} {candidature.etudiant?.lastName}
                    </span>
                </p>
                {candidature.stage && (
                    <p className="text-gray-600">
                        {t('gestionnaire.stage') || 'Stage'}: <span className="font-semibold">
                            {candidature.stage.title}
                        </span>
                    </p>
                )}
            </div>
            ...
            {success && (
                <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded mb-6">
                    {t('gestionnaire.evaluationSauvegardee') || 'Évaluation sauvegardée avec succès !'}
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 space-y-8">
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <div>
                            <label htmlFor="nomEntreprise" className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.nomEntreprise') || 'Nom de l\'entreprise'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                id="nomEntreprise"
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.personneContact') || 'Personne contact'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.adresse') || 'Adresse'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="addresse"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.ville') || 'Ville'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="ville"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.codePostal') || 'Code postal'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="codePostal"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.telephone') || 'Téléphone'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.telecopieur') || 'Télécopieur'}
                            </label>
                            <input
                                type="text"
                                name="telecopieur"
                ...
                <section>
                    ...
                    <div className="grid gap-6 md:grid-cols-3">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.nomStagiaire') || 'Nom du stagiaire'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="nomStagiaire"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.dateStage') || 'Date du stage'} <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="dateStage"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.numeroStage') || 'Numéro de stage'}
                            </label>
                            <input
                                type="number"
                                name="numeroStage"
                ...
                <section>
                    ...
                    <div className="space-y-6">
                        {renderCoteQuestion('tachesCoformes', t('gestionnaire.tachesCoformes') || 'Les tâches confiées correspondent aux objectifs de stage')}
                        {renderCoteQuestion('faciliteIntegration', t('gestionnaire.faciliteIntegration') || 'Facilité d\'intégration dans l\'équipe')}
                        {renderCoteQuestion('tempsEstReel', t('gestionnaire.tempsEstReel') || 'Le temps alloué est réaliste pour les tâches confiées')}
                        {renderCoteQuestion('hygieneRespectable', t('gestionnaire.hygieneRespectable') || 'L\'hygiène est respectable')}
                        {renderCoteQuestion('climatTravailAgreable', t('gestionnaire.climatTravailAgreable') || 'Le climat de travail est agréable')}
                        {renderCoteQuestion('accessibleTransportCommun', t('gestionnaire.accessibleTransportCommun') || 'Accessible en transport en commun')}
                        {renderCoteQuestion('salaireIneteressant', t('gestionnaire.salaireIneteressant') || 'Le salaire est intéressant')}
                        {renderCoteQuestion('communicationSuperviseurFacile', t('gestionnaire.communicationSuperviseurFacile') || 'La communication avec le superviseur est facile')}
                        {renderCoteQuestion('equipementAdequat', t('gestionnaire.equipementAdequat') || 'L\'équipement est adéquat')}
                        {renderCoteQuestion('volumeTravailAcceptable', t('gestionnaire.volumeTravailAcceptable') || 'Le volume de travail est acceptable')}
                    </div>
                    ...
                    <div className="mt-6">
                        ...
                        <textarea
                            ...
                            rows={4}
                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder={t('gestionnaire.commentairesPlaceholder') || 'Ajoutez vos commentaires basés sur vos visites et observations...'}
                        />
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.privilegieStage') || 'Privilégie le stage'}
                            </label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('gestionnaire.nbStagiaires') || 'Nombre de stagiaires'}
                            </label>
                            <input
                                type="number"
                                name="nbStagiaires"
                        ...
                        <div>
                            <label className="flex items-center">
                                ...
                                <span className="text-sm font-medium text-gray-700">
                                    {t('gestionnaire.desireAutreStagiaires') || 'Désire d\'autres stagiaires'}
                                </span>
                            </label>
                        </div>
                        <div>
                            <label className="flex items-center">
                                ...
                                />
                                <span className="text-sm font-medium text-gray-700">
                                    {t('gestionnaire.quartsVariables') || 'Quarts variables'}
                                </span>
                            </label>
                        </div>
                    </div>
                    ...
                    >
                        {saving ? (
                            ...
                            <>
                                ...
                                {t('gestionnaire.sauvegarder') || 'Sauvegarder'}
                            </>
                        )}
                    </button>
                </div>
