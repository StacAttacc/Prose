...
export function NotificationItem({ itemCount, notification, markAndNavigate, markAndClose, typeKey}) {
    ...
    return itemCount <= 3 ? (
            <li key={notification.id} className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded">
                ...
                <div className="flex items-center gap-2">
                    {itemCount <= 3 && (
                        <button
                            type="button"
                            onClick={(e) => markAndClose(e, notification, typeKey)}
                        ...
                        >
                            <svg className="m-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
            ...
            <li key={notification.id} className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded">
                <div className="flex items-start gap-3 flex-1" onClick={(e) => markAndNavigate(e, notification, typeKey)}>
                    {renderCompactItem()}
                </div>
                <div className="flex items-center gap-2">
                    {itemCount >= 4 && (
                        <>
                            <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); markAndClose(e, notification, typeKey); }}
                                className="inline-flex items-center ml-2 py-1 px-2 text-xs font-medium text-gray-700 bg-gray-100 rounded hover:bg-gray-200"
                                aria-label="Mark this notification as read"
                                title="Mark this notification as read"
                            >
